...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField, RadioGroup, FormControlLabel, Radio } from '@mui/material';
...
const AFaq = () => {
  ...
  const [editResponseTime, setEditResponseTime] = useState(new Date().toISOString().slice(0, 16));
  ...
  const handleEdit = (faq_Id) => {
    ...
    axios.get(`${API_URL}/api/faq/${faq_Id}`)
      .then((res) => {
        const faqDetails = res.data;
  ...
  const handleDelete = (faq_Id) => {
    ...
    if (userPassword !== null) {
      axios.delete(`${API_URL}/api/faq/${faq_Id}`, {
  ...
  return (
    <TableContainer component={Paper} sx={{ maxWidth: '90%', margin: 'auto', textAlign: 'center' }}>
      <Table>
        <TableHead>
          <TableRow>
        ...
        <TableBody>
          {axiosResult.map((item, index) => (
            <TableRow key={index}>
              ...
              <TableCell sx={{ overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '200px', whiteSpace: 'nowrap' }}>
                {item.faq_title}
              </TableCell>
              <TableCell sx={{ overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '200px', whiteSpace: 'nowrap' }}>
                {item.faq_content}
              ...
              <TableCell>
                ...
                <Button variant="contained" color="error" onClick={() => handleDelete(item.faq_id)}>
                  삭제
                </Button>
              </TableCell>
            </TableRow>
      ...
      <Dialog open={openEditDialog} onClose={handleCancelEdit} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          ...
          >
            ...
            <FormControlLabel value="true" control={<Radio />} label="답변 완료" />
          </RadioGroup>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCancelEdit}>취소</Button>
          <Button onClick={handleSaveEdit} color="primary">저장</Button>
        </DialogActions>
      </Dialog>
    </TableContainer>
  );
